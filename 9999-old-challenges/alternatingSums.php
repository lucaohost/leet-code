...
// an array of two integers, where the first element is the total weight of team 1, and 
...
function alternatingSums($people)
{
    $teamOneWeight = 0;
    $teamTwoWeight = 0;
    foreach ($people as $position => $weight) {
        $position % 2 === 0 ? $teamOneWeight += $weight : $teamTwoWeight += $weight;
    }
    return [$teamOneWeight, $teamTwoWeight];
}

var_dump(alternatingSums([50, 60, 60, 45, 70]));
